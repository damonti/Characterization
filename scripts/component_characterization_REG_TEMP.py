...
for TOGGLE in TOGGLES: #take one toggle rate
    for rep in REPETITIONS: #repeat each toggle rate 100 times so that interleaving is averaged
        ...
        os.system("python3 "+SCRIPT_PATH+"/component_stim_vector_gen.py "+DESIGN+" "+UNIT+" "+str(TOGGLE)+" "+str(BW))
 
        #update run.tcl with the new toggle rate
        with open(SIM_PATH+'/run_xcelium/run.tcl', 'w') as wr:
            wr.write("dumptcf -output "+SIM_PATH+"/tcf/"+UNIT+"_"+str(TOGGLE)+"percent_#"+str(rep)+".tcf -scope /tb_"+UNIT+" -dumpportsonly -eot -overwrite \nrun \ndumptcf -end \nexit")
        wr.close()
