        ...
        #  print( [m for m in line if m.startswith("fir_Mul_")])
            ...
            #    weights = [x*min_w for x in range(1, how_many_input+1)] #list of weights 0.12, 0.25, ...
                ...
                for j in range(0, how_many_input):
                    ...
                    #print(temp.split("  ")[-1])
                    input_values.append(round(int(temp.split(" ")[-1])*weights[weight_index],4)) 
                ...
                for bit in range(0,len(input_values)):
                    input_instance_activity = round(((input_values[bit]/(clock_toggles))*100),2)
                    print("\nb"+bit+" activity = "+str(input_instance_activity))
