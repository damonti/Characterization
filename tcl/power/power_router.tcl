...
###############################
#RTL
::legacy::set_attribute init_hdl_search_path $RTL_PATH/hdl
read_netlist $RTL_PATH/netlist/${DESIGN}_${BW}BW_netlist.v
...
set END 1546
set BW 64
set PERCENT 25percent
set TB "router_${PERCENT}_${BW}BW"
read_stimulus -start ${START}ns -end ${END}ns -allow_n_nets -format vcd -file $SIM_PATH/dump_${TB}.vcd -dut_instance /noc_test/noc/n0
compute_power -mode time_based
report_power  -levels all -by_hierarchy -indent_inst -levels all -header -cols "cells static internal switching dynamic total" -unit nW > ${REPORT_PATH}/router.${TB}_payload.rpt

suspend

set PERCENT 100percent
set START 1536
set END 1548
set TB "router_${PERCENT}_${BW}BW"
read_stimulus -start ${START}ns -end ${END}ns -allow_n_nets -format vcd -file $SIM_PATH/dump_${TB}.vcd -dut_instance /noc_test/noc/n0
